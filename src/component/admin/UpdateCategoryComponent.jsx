...
const UpdateCategoryComponent = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategory = async () => {
      try {
        setLoading(true);
        const response = await GenreService.getGenreById(id.categoryId);
        console.log(response);
        
        const { genreName, genreNameKh, description, image } = response.data;
        form.setFieldsValue({ genreName, genreNameKh, description });
        setImagePath(image);
        setLoading(false);
      } catch (error) {
  ...
  const onFinish = async (values) => {
    try {
      ...
      await GenreService.updateGenre(id.categoryId, data);
  ...
  return (
    <div className="p-6">
      ...
      <h1 className="text-2xl font-semibold text-blue-500">Update Category</h1>
